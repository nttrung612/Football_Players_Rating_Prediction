...
class InternalDecisionTree:
    """
    Đối tượng cây decision tree bình thường (cá thể) để tối ưu.
    """
    ...
    def copy_from_dt(self, dt):
        """
            copy contents cua scikit-learn decision tree
        """
        ...
        if self.target_type == 'classification':
            # cây decision tree của scikit-learn lưu xác suất cho từng lớp trong mảng,
            # cây decision tree này chỉ lưu dự đoán cho mỗi phần tử node_prediction
            # dt có thể có thứ tự khác với GeneticDecisionTree, nhưng ở đây chúng ta lấy dự đoán dựa trên trên thứ tự trong dt.
    ...
    def count_training_records(self, x, y):
        ...
        # Lặp qua các record. Với mỗi record, xác định node lá mà nó kết thúc. Tăng giá trị count nút lá đấy lên một giá trị = Y.
        for i in x.index:
        ...
        # Tính xong count, xác định dự đoán cho từng nút lá
    ...
    def find_means(self, x, y):
        ...
        # lặp qua các record. Với mỗi record, xác định node mà nó kết thúc. Tăng giá trị sum và count nút lá đấy lên một giá trị = Y.
        for i in x.index:
            row = x.loc[i]
            cur_node_idx = 0
            while self.feature[cur_node_idx] >= 0:
...
class GeneticDecisionTree:
    def __init__(self,
        ...
        :param max_depth: int
            Độ sâu tối da của cây. Độ sâu càng thấp càng đơn giản nhưng kém chính xác.

        :param max_iterations: int
            Số lần lặp thuật toán thực hiện. Mỗi lần lặp, một số câu được sinh ra. Kết thúc mỗi lần lặp thì ta sẽ
            lưu lại cá thể tốt nhất để tiếp tục tối ưu quần thể.

        :param allow_mutate: bool
            Có thực hiện đột biến hay không.
                + Không, để giảm thời gian chạy thuật toán, có khả năng quá khớp
                + Có, các chỉ số tồn tại trong cây có thể được tạo ra bằng cách sửa đổi ngưỡng thành giá trị ngẫu nhiên
                trên các nút ngẫu nhiên

        :param allow_combine: bool
            Có thực hiện lai ghép hay không.
                + Không, để giảm thời gian chạy thuật toán, có khả năng quá khớp
                + Có, lai ghép bằng cách kết hợp cây con trái của một node và cây con trái của một node khác.

        :param n_jobs: int
            Số lượng tiến trình được tạo.

        :param random_state: int

        :param verbose: bool
            Đặt = true để một số đầu ra được hiển thị trong quá trình khớp
        """
